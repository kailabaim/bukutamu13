...
@section('title', 'Daftar Tamu Umum- Tamuin')
@section('page-title', 'Daftar Tamu Umum')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center justify-between mb-6">
            <form method="GET" action="{{ route('tamu_umum.index') }}" class="flex items-center gap-3 w-full">
                {{-- Search --}}
                ...
                <a href="{{ route('tamu_umum.export.excel', ['bulan' => $bulan, 'search' => $search, 'sort' => $sort]) }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    @forelse($tamu_umum as $item)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="py-3 px-4">
                            <span class="font-medium text-gray-900">{{ $item->nama }}</span>
                        </td>
                        <td class="py-3 px-4 text-gray-700">{{ $item->identitas }}</td>
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('tamu_umum.edit', $item) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form method="POST" action="{{ route('tamu_umum.destroy', $item) }}" onsubmit="return confirm('Hapus data ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="flex items-center justify-between mt-4">
            <p class="text-sm text-gray-600">Showing {{ $tamu_umum->firstItem() ?? 0 }} from {{ $tamu_umum->total() }} data</p>
            <div class="flex items-center space-x-2">
                {{ $tamu_umum->links() }}
            </div>
        </div>
